...
def report(out_dir: pathlib.Path, allow_missing_x11: bool) -> int:
    ...
    for text in (
        ...
        "DRM client cap ATOMIC value 2 is accepted",
        ...
        "DRM cursor ioctl rejects bad CRTC id",
        "DRM cursor ioctl bad CRTC id fails with ENOENT",
        "DRM cursor2 ioctl rejects bad CRTC id",
        "DRM cursor2 ioctl bad CRTC id fails with ENOENT",
        "WAIT_VBLANK rejects SIGNAL requests",
        ...
        "PAGE_FLIP rejects both target flags",
        "PAGE_FLIP both target flags fail with EINVAL",
        "PAGE_FLIP rejects bad CRTC id",
        "PAGE_FLIP bad CRTC id fails with ENOENT",
        "PAGE_FLIP lookup probe reads resources",
        "PAGE_FLIP lookup probe finds active CRTC",
        "PAGE_FLIP lookup probe active CRTC index is valid",
        "PAGE_FLIP lookup probe reads active CRTC",
        "PAGE_FLIP lookup probe active CRTC has framebuffer",
        "PAGE_FLIP rejects bad framebuffer id",
        "PAGE_FLIP bad framebuffer id fails with ENOENT",
        "DRM GETPROPERTY rejects bad property id",
        "DRM GETPROPERTY bad property id fails with ENOENT",
        "DRM OBJ_GETPROPERTIES rejects bad object id",
        "DRM OBJ_GETPROPERTIES bad object id fails with ENOENT",
        "DRM SETPROPERTY rejects bad connector id",
        "DRM SETPROPERTY bad connector id fails with ENOENT",
        "DRM OBJ_SETPROPERTY rejects bad object id",
        ...
        "DRM SETPROPERTY rejects bad property id",
        "DRM SETPROPERTY bad property id fails with EINVAL",
        "DRM OBJ_SETPROPERTY rejects bad property id",
        "DRM OBJ_SETPROPERTY bad property id fails with EINVAL",
        "DRM GETCRTC rejects bad CRTC id",
        "DRM GETCRTC bad CRTC id fails with ENOENT",
        "DRM GETCONNECTOR rejects bad connector id",
        "DRM GETCONNECTOR bad connector id fails with ENOENT",
        "DRM GETENCODER rejects bad encoder id",
        "DRM GETENCODER bad encoder id fails with ENOENT",
        "DRM GETPLANE rejects bad plane id",
        "DRM GETPLANE bad plane id fails with ENOENT",
        "SETCRTC lookup probe reads resources",
        "SETCRTC lookup probe finds active CRTC",
        "SETCRTC lookup probe active CRTC index is valid",
        "SETCRTC lookup probe reads active CRTC",
        "SETCRTC lookup probe finds active mode",
        "SETCRTC lookup probe active mode has size",
        "SETCRTC rejects bad CRTC id",
        "SETCRTC bad CRTC id fails with ENOENT",
        "SETCRTC rejects bad framebuffer id",
        ...
        "SETCRTC rejects bad connector id",
        ...
        "SETCRTC rejects high x source offset",
        "SETCRTC high x source offset fails with ERANGE",
        "SETCRTC rejects high y source offset",
        "SETCRTC high y source offset fails with ERANGE",
        "SETCRTC argument probe reads resources",
        "SETCRTC argument probe finds active CRTC",
        "SETCRTC argument probe active CRTC index is valid",
        "SETCRTC argument probe reads active CRTC",
        "SETCRTC argument probe finds active mode",
        "SETCRTC argument probe active mode has size",
        "CREATE_DUMB succeeds for SETCRTC argument probe",
        "ADDFB2 accepts SETCRTC argument probe framebuffer",
        "SETCRTC rejects mode without connectors",
        "SETCRTC mode without connectors fails with EINVAL",
        "SETCRTC rejects connectors without mode or fb",
        "SETCRTC connectors without mode or fb fail with EINVAL",
        "SETCRTC rejects excessive connector count",
        "SETCRTC excessive connector count fails with EINVAL",
        "RMFB succeeds for SETCRTC argument probe framebuffer",
        "DESTROY_DUMB succeeds for SETCRTC argument probe BO",
        "SETPLANE lookup probe reads plane resources",
        "SETPLANE lookup probe finds visible plane",
        "SETPLANE rejects bad plane id",
        "SETPLANE bad plane id fails with ENOENT",
        "SETPLANE rejects bad framebuffer id",
        "SETPLANE bad framebuffer id fails with ENOENT",
        "CREATE_DUMB succeeds for SETPLANE lookup probe",
        "ADDFB2 accepts SETPLANE lookup probe framebuffer",
        "SETPLANE rejects bad CRTC id",
        "SETPLANE bad CRTC id fails with ENOENT",
        "RMFB succeeds for SETPLANE lookup probe framebuffer",
        "DESTROY_DUMB succeeds for SETPLANE lookup probe BO",
        "SETPLANE argument probe reads resources",
        "SETPLANE argument probe finds active CRTC",
        "SETPLANE argument probe active CRTC index is valid",
        "SETPLANE argument probe reads plane resources",
        "SETPLANE argument probe finds active primary plane",
        "CREATE_DUMB succeeds for SETPLANE argument probe",
        "ADDFB2 accepts SETPLANE argument probe framebuffer",
        "SETPLANE rejects overflowing CRTC x+w",
        "SETPLANE overflowing CRTC x+w fails with ERANGE",
        "SETPLANE rejects overflowing CRTC width",
        "SETPLANE overflowing CRTC width fails with ERANGE",
        "SETPLANE rejects source rect outside framebuffer",
        ...
        "DRM property blob rejects zero length create",
        "DRM property blob zero length create fails with EINVAL",
        "DRM property blob create succeeds",
        "DRM property blob create returns non-zero id",
        "DRM property blob length query succeeds",
        ...
        "DRM property blob destroyed id is unreadable",
        "DRM property blob destroyed id read fails with ENOENT",
        "DRM property blob double destroy is rejected",
        ...
        "CREATE_DUMB lifecycle probe succeeds",
        "CREATE_DUMB returns non-zero handle",
        "CREATE_DUMB pitch covers requested width",
        "CREATE_DUMB size covers requested pitch and height",
        "MAP_DUMB owner handle succeeds",
        ...
        "connector fill_modes counter is present before master GETCONNECTOR reprobe",
        "master GETCONNECTOR count_modes=0 succeeds",
        "master GETCONNECTOR returns requested connector",
        "master GETCONNECTOR returns cached modes",
        "connector fill_modes counter is present before master GETCONNECTOR reprobe result",
        "master GETCONNECTOR count_modes=0 triggers connector fill_modes",
        "non-master GETCONNECTOR opens secondary card fd",
        "connector fill_modes counter is present before non-master GETCONNECTOR reprobe",
        "non-master GETCONNECTOR count_modes=0 succeeds",
        "non-master GETCONNECTOR returns requested connector",
        "non-master GETCONNECTOR returns cached modes",
        "connector fill_modes counter is present before non-master GETCONNECTOR reprobe result",
        ...
        "legacy GETGAMMA active CRTC gamma size is non-zero",
        "legacy GETGAMMA allocates owner readback ramps",
        "legacy GETGAMMA rejects bad CRTC id",
        "legacy GETGAMMA bad CRTC id fails with ENOENT",
        "legacy GETGAMMA owner read succeeds",
        "legacy GETGAMMA rejects wrong gamma size",
        "legacy GETGAMMA wrong gamma size fails with EINVAL",
        "legacy GETGAMMA rejects bad red pointer",
        ...
        "legacy SETGAMMA active CRTC gamma size is non-zero",
        "legacy SETGAMMA allocates error probe ramps",
        "legacy SETGAMMA rejects bad CRTC id",
        ...
        "PRIME_HANDLE_TO_FD rejects unknown flags",
