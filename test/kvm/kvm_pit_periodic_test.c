 ...
 *
 * Verifies that a low-frequency periodic PIT channel continues to raise IRQ0
 * after each guest PIC EOI.
 */
#include <sys/mman.h>
#include <sys/types.h>
#include <sys/ioctl.h>
...
kvm_pit_write_irq0_handler(uint8_t *memory)
{
	static const uint8_t stub_code[] = {
		...
		0xb0, 0x34, 0xe6, 0x43,	/* PIT ch0: lo/hi, mode 2 */
	...
	static const uint8_t handler_code[] = {
		0xfe, 0x06, 0x00, 0x03,		/* incb 0x300 */
...
main(void)
{
	...
	if (run->exit_reason != KVM_EXIT_HLT)
		errx(1, "expected HLT after periodic PIT IRQ0, got %u",
		    run->exit_reason);
	if (guest_low_memory[KVM_PIT_RESULT_OFFSET] < 2)
		errx(1, "periodic PIT IRQ0 count %u",
		    guest_low_memory[KVM_PIT_RESULT_OFFSET]);
