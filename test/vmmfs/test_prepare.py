...
int main(void) {
    for (mode = 0; mode < 14; ++mode) {
        ...
        machine.vcpu.node.vnode = mode == 13 ? NULL : &cpu_vnode;
        ...
        assert(cpu_vnode.refs == (mode == 13 ? 0 : 1));
        assert(machine.vcpu.token.live == (mode != 13));
        if ((mode >= 1 && mode <= 4) || mode == 10 || mode == 13) {
