...
cleanup()
{
	set +e; cleanup_machine vmmcall; cleanup_machine cpuid; cleanup_machine serial; cleanup_machine time; cleanup_machine xsetbv; cleanup_machine apicmsr; cleanup_machine hlt; cleanup_machine loop
	[ "$MOUNTED" -eq 1 ] && umount "$MNT" >>"$LOG" 2>&1 && MOUNTED=0
	[ "$LOADED" -eq 1 ] && [ "$MOUNTED" -eq 0 ] && kldunload vmm >>"$LOG" 2>&1
	rm -f /var/tmp/vmmld_smoke_vmmcall /var/tmp/vmmld_smoke_cpuid /var/tmp/vmmld_smoke_serial /var/tmp/vmmld_smoke_time /var/tmp/vmmld_smoke_xsetbv /var/tmp/vmmld_smoke_apicmsr /var/tmp/vmmld_smoke_hlt /var/tmp/vmmld_smoke_loop
}
...
run_case vmmcall; run_case cpuid; run_case serial; run_case time; run_case xsetbv; run_case apicmsr; run_case hlt; run_case loop; say "PASS"
