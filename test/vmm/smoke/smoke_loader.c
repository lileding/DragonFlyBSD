...
{
	uint8_t bytes[] = { 0x66, 0xba, port & 0xffU, port >> 8 };
...
static size_t
guest_hires_lapictimer_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_edi_apic[] = { 0xbf, 0x00, 0x00, 0xe0, 0xfe };
	static const uint8_t mov_eax_to_tdcr[] =
	    { 0x89, 0x87, 0xe0, 0x03, 0x00, 0x00 };
	...
	static const uint8_t mov_eax_to_esi[] = { 0x89, 0xc6 };
	...
	emit(code, &len, cap, mov_eax_to_esi, sizeof(mov_eax_to_esi));
	...
	emit(code, &len, cap, mov_edi_apic, sizeof(mov_edi_apic));
	emit(code, &len, cap, rdtsc, sizeof(rdtsc));
	emit(code, &len, cap, sub_esi_from_eax, sizeof(sub_esi_from_eax));
	emit(code, &len, cap, mov_eax_to_ecx, sizeof(mov_eax_to_ecx));
	emit_mov_eax(code, &len, cap, 0);
	emit(code, &len, cap, mov_eax_to_eoi, sizeof(mov_eax_to_eoi));
...
static size_t
guest_hires_tscdeadline_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_edi_apic[] = { 0xbf, 0x00, 0x00, 0xe0, 0xfe };
	static const uint8_t mov_eax_to_lvtt[] =
	    { 0x89, 0x87, 0x20, 0x03, 0x00, 0x00 };
...
static size_t
guest_smptriplefault_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_edi_apic[] = { 0xbf, 0x00, 0x00, 0xe0, 0xfe };
	static const uint8_t mov_eax_to_icr_high[] =
	    { 0x89, 0x87, 0x10, 0x03, 0x00, 0x00 };
	static const uint8_t mov_eax_to_icr_low[] =
	    { 0x89, 0x87, 0x00, 0x03, 0x00, 0x00 };
	static const uint8_t delay_then_ud[] = {
	    0xb9, 0x00, 0xe1, 0xf5, 0x05, /* mov ecx, 100000000 */
	    0xff, 0xc9,                   /* dec ecx */
	    0x75, 0xfc,                   /* jnz dec ecx */
	    0x0f, 0x0b                    /* ud2 with an empty IDT */
	};
	size_t len = 0;

	/* Start APIC ID 1, then fault after the AP has reached its HLT loop. */
	emit(code, &len, cap, mov_edi_apic, sizeof(mov_edi_apic));
	emit_mov_eax(code, &len, cap, 1U << 24);
	emit(code, &len, cap, mov_eax_to_icr_high,
	    sizeof(mov_eax_to_icr_high));
	...
	emit_mov_eax(code, &len, cap, 0x00000608U);
	emit(code, &len, cap, mov_eax_to_icr_low, sizeof(mov_eax_to_icr_low));
	emit(code, &len, cap, delay_then_ud, sizeof(delay_then_ud));
	return len;
}
...
static size_t
guest_ioapicirq_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_edi_ioapic[] =
	    { 0xbf, 0x00, 0x00, 0xc0, 0xfe };
	...
	emit_mov_eax(code, &len, cap, 0x18);
	emit(code, &len, cap, mov_eax_to_rdi, sizeof(mov_eax_to_rdi));
	emit_mov_eax(code, &len, cap, IOAPIC_VECTOR);
	emit(code, &len, cap, mov_eax_to_rdi_10,
	    sizeof(mov_eax_to_rdi_10));
	emit_mov_eax(code, &len, cap, AVIC_MAGIC);
	emit(code, &len, cap, (const uint8_t[]){ 0xbb }, 1);
...
guest_x2apic_code(uint8_t *code, size_t cap)
{
	static const uint8_t setup[] = {
	    ...
	    0x0d, 0x00, 0x0d, 0x00, 0x00, /* or eax,BSP|X2APIC|EN */
	    0x0f, 0x30,		/* wrmsr */
	    0xb9, 0x08, 0x08, 0x00, 0x00, /* mov ecx,x2APIC TPR */
	    0xb8, 0x20, 0x00, 0x00, 0x00, /* mov eax,0x20 */
	    0x31, 0xd2,		/* xor edx,edx */
	    0x0f, 0x30,		/* wrmsr */
	    0x0f, 0x32,		/* rdmsr */
	    0x3d, 0x20, 0x00, 0x00, 0x00	/* cmp eax,0x20 */
	};
	static const uint8_t fail[] = { 0xf4, 0xeb, 0xfe };
...
static size_t
guest_elcr_code(uint8_t *code, size_t cap)
{
	static const uint8_t seq[] = {
	    0xba, PIC_ELCR1_PORT & 0xffU,
	    (PIC_ELCR1_PORT >> 8) & 0xffU, 0x00, 0x00,
	    0xb0, 0xff,			/* mov al,0xff */
	    0xee,			/* out dx,al */
	    0xec,			/* in al,dx */
	    0x3c, 0xf8,			/* cmp al,0xf8 */
	    0x75, 0x10,			/* jne fail */
	    0xba, PIC_ELCR2_PORT & 0xffU,
	    (PIC_ELCR2_PORT >> 8) & 0xffU, 0x00, 0x00,
	    0xb0, 0xff,			/* mov al,0xff */
	    0xee,			/* out dx,al */
	    0xec,			/* in al,dx */
...
static size_t
guest_hpet_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_edi_hpet[] =
	    { 0xbf, 0x00, 0x00, 0xd0, 0xfe };
	static const uint8_t mov_rdi_to_eax[] = { 0x8b, 0x07 };
	static const uint8_t mov_rdi_4_to_eax[] = { 0x8b, 0x47, 0x04 };
	static const uint8_t mov_eax_to_rdi_10[] = { 0x89, 0x47, 0x10 };
	static const uint8_t mov_counter_to_eax[] =
	    { 0x8b, 0x87, 0xf0, 0x00, 0x00, 0x00 };
	static const uint8_t mov_eax_to_ebx[] = { 0x89, 0xc3 };
	static const uint8_t cmp_eax_ebx[] = { 0x39, 0xd8 };
	static const uint8_t pause_op[] = { 0xf3, 0x90 };
	static const uint8_t jmp8[] = { 0xeb, 0x00 };
	static const uint8_t fail[] = { 0xf4, 0xeb, 0xfe };
	size_t len = 0;
	...
	emit(code, &len, cap, mov_rdi_to_eax, sizeof(mov_rdi_to_eax));
	emit_cmp_eax(code, &len, cap, HPET_CAP_LOW);
	jcap = emit_jne32(code, &len, cap);
	emit(code, &len, cap, mov_rdi_4_to_eax, sizeof(mov_rdi_4_to_eax));
	emit_cmp_eax(code, &len, cap, HPET_PERIOD_FS);
	jperiod = emit_jne32(code, &len, cap);
	emit_mov_eax(code, &len, cap, 1);
	emit(code, &len, cap, mov_eax_to_rdi_10, sizeof(mov_eax_to_rdi_10));
	emit(code, &len, cap, mov_counter_to_eax, sizeof(mov_counter_to_eax));
	emit(code, &len, cap, mov_eax_to_ebx, sizeof(mov_eax_to_ebx));
	loop_label = len;
	emit(code, &len, cap, mov_counter_to_eax, sizeof(mov_counter_to_eax));
	emit(code, &len, cap, cmp_eax_ebx, sizeof(cmp_eax_ebx));
	jchanged = emit_jne32(code, &len, cap);
	emit(code, &len, cap, pause_op, sizeof(pause_op));
	emit(code, &len, cap, jmp8, sizeof(jmp8));
	jmp_back = len - 1;
	ok_label = len;
	...
	patch_rel32(code, jperiod, fail_label);
...
guest_pmtimer_code(uint8_t *code, size_t cap)
{
	static const uint8_t mov_dx_pm_timer[] =
	    { 0x66, 0xba, PM_TIMER_PORT & 0xff, PM_TIMER_PORT >> 8 };
	...
	emit(code, &len, cap, mov_dx_pm_timer, sizeof(mov_dx_pm_timer));
	emit(code, &len, cap, in_eax_dx, sizeof(in_eax_dx));
	emit(code, &len, cap, mov_eax_to_ebx, sizeof(mov_eax_to_ebx));
	loop_label = len;
	emit(code, &len, cap, in_eax_dx, sizeof(in_eax_dx));
	emit(code, &len, cap, cmp_eax_ebx, sizeof(cmp_eax_ebx));
...
guest_code(const char *mode, uint8_t *code, size_t cap)
{
	...
	static const uint8_t triplefault[] = { 0x0f, 0x0b };
	const uint8_t *src;
	size_t len;
	...
	if (strcmp(mode, "vmmcall") == 0) {
		...
		return guest_ioapicirq_code(code, cap);
	...
	} else if (strcmp(mode, "smptriplefault") == 0) {
		return guest_smptriplefault_code(code, cap);
	...
	} else if (strcmp(mode, "elcr") == 0) {
		return guest_elcr_code(code, cap);
	} else if (strcmp(mode, "hpet") == 0) {
		...
		src = cliloop;
		len = sizeof(cliloop);
	} else if (strcmp(mode, "triplefault") == 0) {
		src = triplefault;
		len = sizeof(triplefault);
	} else {
...
build_guest(uint8_t *mem, size_t mem_size, const char *mode, size_t *code_len)
{
	...
	if (strcmp(mode, "timerint") == 0 ||
		...
		if (strcmp(mode, "hpet_oneshot") == 0 ||
			...
			write_idt_gate(mem, IOAPIC_VECTOR, AVIC_HANDLER_GPA);
		else if (strcmp(mode, "avicirq") == 0)
