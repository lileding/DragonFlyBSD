 ...
 *
 * Provider for the P7 reset-generation DMA capability harness.
 */
...
#define VMM_PCIE_DMA_RESET_RUNS	2U
...
main(int argc, char **argv)
{
	...
	for (run = 1; run <= VMM_PCIE_DMA_RESET_RUNS; run++) {
		receive_start(fd, &start_message);
		generation = le64toh(start_message.le_memory_generation);
		if (start_message.header.le_sequence !=
	    start_message.le_memory_generation || generation <= previous_generation)
			errno = EPROTO, err(1, "non-increasing DMA generation");
		...
		printf("DFVMM_PCIE_DMA_RESET_PROVIDER_READY run=%u generation=%ju\n",
		    run, (uintmax_t)generation);
		if (fflush(stdout) != 0)
			err(1, "flush ready");
		...
		expect_revoked(dma);
		expect_revoked(bar);
		printf("DFVMM_PCIE_DMA_RESET_PROVIDER_REVOKED run=%u generation=%ju\n",
		    run, (uintmax_t)generation);
		if (fflush(stdout) != 0)
			err(1, "flush revoke");
	}
...
build_register(struct vmm_pcie_abi_register *message, uint64_t generation)
{
	...
	message->le_device_id = htole16(0xdf08);
	message->le_subsystem_vendor_id = htole16(0x1b36);
	message->le_subsystem_device_id = htole16(0xdf08);
