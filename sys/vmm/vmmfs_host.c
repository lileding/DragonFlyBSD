 ...
 *
 * Filesystem presentation of the host: machines/host/, a permanent read-only
 * directory whose only child is devices/ (the host PCIe pool).  Unlike user VMs
 * it is never created or removed.  vmm.ko only.
 */
#include <sys/param.h>
#include <sys/systm.h>
...
static int
vmmfs_host_readdir(struct vmmfs_node *node, struct vop_readdir_args *ap)
{
