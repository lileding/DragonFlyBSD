...
#define _BITULL(nr) (1ULL << (nr))
#define __GENMASK_ULL(high, low) \
    (((~0ULL) << (low)) & (~0ULL >> (63 - (high))))
...
/* DragonFly fixed-size indirection for variable-length KVM payloads. */
...
#define KVM_DFLY_EVENTFD_VALID_FLAGS \
    (KVM_DFLY_EVENTFD_NONBLOCK | KVM_DFLY_EVENTFD_CLOEXEC)
...
static inline int
kvm_dfly_eventfd(unsigned int initial, int flags)
{
    ...
    if ((flags & EFD_CLOEXEC) != 0)
        request.flags |= KVM_DFLY_EVENTFD_CLOEXEC;
    if ((flags & EFD_NONBLOCK) != 0)
        request.flags |= KVM_DFLY_EVENTFD_NONBLOCK;
    request.fd = -1;
