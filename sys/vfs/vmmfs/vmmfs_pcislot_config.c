...
vmmfs_pcislot_config_fini(struct vmmfs_pcislot_config *config)
{
	...
	if (slot->pciroot == NULL)
		panic("vmmfs_pcislot_config_fini: slot lost its PCI root");
	machine = slot->pciroot->machine;
	if (machine == NULL)
...
vmmfs_pcislot_config_getattr(struct vop_getattr_args *ap)
{
	...
	if (config == NULL || config->slot == NULL ||
	    vmmfs_pcislot_is_dead(config->slot))
		return (ENOENT);
	vattr = ap->a_vap;
	VATTR_NULL(vattr);
	vattr->va_type = VREG;
...
vmmfs_pcislot_config_inactive(struct vop_inactive_args *ap)
{
	...
	if (machine == NULL || !vmmfs_pcislot_is_dead(config->slot))
		return (0);
	...
	uint64_t offset, enum vmm_io_width width, uint64_t *match_offset)
{
	...
	if (config == NULL || resource == NULL || match_offset == NULL ||
		...
		return (EINVAL);
	value = &config->slot->descriptor.value;
