	...
	struct vmmfs_pcislot **objectp)
{
	...
	*objectp = NULL;
	...
	slot->node.size = 0;
	error = vmmfs_pcislot_powered_init(&slot->node, &slot->powered);
	if (error != 0)
		goto fail;
...
fail:
	...
	(void)vmmfs_node_deactivate(&slot->powered.node);
...
vmmfs_pcislot_drop(struct vmmfs_node *node)
{
	...
	KKASSERT(slot->powered.node.drop == NULL);
	slot->bdf = 0;
	lwkt_token_uninit(&slot->token);
	kfree(slot, M_VMMFS);
}
...
vmmfs_pcislot_deactivate(struct vmmfs_node *node)
{
	...
	int error;
	struct vmmfs_pciroot *parent =
	    (struct vmmfs_pciroot *)node->parent;
...
vmmfs_pcislot_power_on(struct vmmfs_pcislot *slot, vmm_machine_t machine)
{
	...
	if (slot == NULL || machine == NULL || !slot->descriptor.committed || slot->resources != NULL ||
	    slot->type0.powered)
		return (EINVAL);
	...
	vmmfs_pcislot_powered_set(&slot->powered, true);
	return (0);
}
...
vmmfs_pcislot_power_off(struct vmmfs_pcislot *slot)
{
	...
	if (slot == NULL)
		return;
	vmmfs_pcislot_powered_set(&slot->powered, false);
	vmmfs_pciroot_invalidate_slot(vmmfs_pcislot_pciroot(slot), slot);
	...
	const char *name, size_t length, struct vnode **vnodep)
{
	...
	if (length == sizeof("powered") - 1 &&
	    bcmp(name, "powered", sizeof("powered") - 1) == 0)
		vnode = slot->powered.node.vnode;
	...
	struct vmmfs_node_item *item)
{
	...
	if (index == 0) {
		item->inode = slot->powered.node.inode;
		item->type = DT_REG;
		bcopy("powered", item->name, sizeof("powered"));
		lwkt_reltoken(&slot->token);
		return (0);
	}
	--index;
