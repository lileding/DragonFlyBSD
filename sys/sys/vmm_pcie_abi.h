 ...
 */
#ifndef _SYS_VMM_PCIE_ABI_H_
#define _SYS_VMM_PCIE_ABI_H_
...
#define VMM_PCIE_ABI_MAX_BAR_RANGES		8U
...
/* Ranges are page-aligned, non-overlapping and cover every non-MSI-X BAR page. */
...
struct vmm_pcie_abi_register {
	...
	uint8_t		bar_range_count;
	uint8_t		reserved1[2];
	struct vmm_pcie_abi_vendor_cap
			vendor_cap[VMM_PCIE_ABI_MAX_VENDOR_CAPS];
	struct vmm_pcie_abi_bar_range
			bar_range[VMM_PCIE_ABI_MAX_BAR_RANGES];
} __attribute__((__packed__));
...
/* Provider RPC for BAR pages declared VMM_PCIE_ABI_BAR_RANGE_F_TRAPPED. */
struct vmm_pcie_abi_mmio {
	struct vmm_pcie_abi_header header;
	uint64_t	le_device_id;
	uint64_t	le_attachment_generation;
	uint64_t	le_request_id;
	uint64_t	le_offset;
	uint64_t	le_value;
	uint32_t	le_bar_index;
	uint32_t	le_size;
...
_Static_assert(sizeof(struct vmm_pcie_abi_bar) == 16,
    "vPCIe ABI BAR size");
_Static_assert(sizeof(struct vmm_pcie_abi_bar_range) == 24,
    "vPCIe ABI BAR range size");
...
_Static_assert(sizeof(struct vmm_pcie_abi_register) == 852,
...
_Static_assert(sizeof(struct vmm_pcie_abi_mmio) == 80,
    "vPCIe ABI MMIO size");

int	vmm_pcie_abi_validate(const void *message, size_t size);

#endif /* _SYS_VMM_PCIE_ABI_H_ */
