 ...
 *
 * VMX APICv interrupt controller.
 *
 * This implementation owns hardware LAPIC, IOAPIC, MSI and legacy interrupt
 * state for a VMX machine with an irqchip but with APICv.  It never owns a
...
#define VMM_VMX_APICV_LAPIC_BASE		0xfee00000ULL
#define VMM_VMX_APICV_MAX_APIC_ID	0xfeU
...
#define VMM_VMX_APICV_APICBASE_VALID	(VMM_VMX_APICBASE_BSP | \
...
struct vmm_vmx_interrupt_machine {
	...
	vm_page_t apic_access_page;
	paddr_t apic_access_page_pa;
	bool apic_access_mapped;
	uint32_t ioapic_select;
	uint32_t ioapic_id;
	uint64_t ioapic_redir[VMM_VMX_IOAPIC_PINS];
	bool ioapic_level[VMM_VMX_IOAPIC_PINS];
	struct vmm_vmx_interrupt_vcpu *targets[VMM_VMX_APICV_MAX_APIC_ID + 1];
};
...
static int vmm_vmx_apicv_machine_create(struct vmm_machine *,
    struct vmm_vmx_interrupt_machine **);
static int vmm_vmx_apicv_machine_enable(struct vmm_vmx_interrupt_machine *);
static int vmm_vmx_apicv_irq_raise_msi(struct vmm_vmx_interrupt_machine *,
    uint64_t, uint32_t);
static int vmm_vmx_apicv_irq_set(struct vmm_vmx_interrupt_machine *, uint32_t,
    bool);
...
static int vmm_vmx_apicv_vcpu_mmio(struct vmm_vmx_interrupt_vcpu *, uint64_t,
    bool, uint32_t *);
static int vmm_vmx_apicv_vcpu_io(struct vmm_vcpu *,
    const struct vmm_cpuexit_io *);
static int vmm_vmx_apicv_vcpu_msr(struct vmm_vmx_interrupt_vcpu *, bool,
    uint32_t, uint64_t *);
static void vmm_vmx_apicv_machine_destroy(struct vmm_vmx_interrupt_machine *);
static int vmm_vmx_apicv_vcpu_create(struct vmm_vmx_interrupt_machine *,
    struct vmm_vcpu *, struct vmm_vmx_interrupt_vcpu **,
    struct vmm_vmx_interrupt_config *);
static void vmm_vmx_apicv_vcpu_destroy(struct vmm_vmx_interrupt_vcpu *);
static int vmm_vmx_apicv_vcpu_prepare(struct vmm_vmx_interrupt_vcpu *);
static void vmm_vmx_apicv_vcpu_enter(struct vmm_vmx_interrupt_vcpu *);
static void vmm_vmx_apicv_vcpu_leave(struct vmm_vmx_interrupt_vcpu *);
static void vmm_vmx_apicv_vcpu_event_result(
    struct vmm_vmx_interrupt_vcpu *, bool);
static int vmm_vmx_apicv_vcpu_exit(struct vmm_vmx_interrupt_vcpu *, uint64_t,
    uint64_t, uint64_t);
static int vmm_vmx_apicv_route_icr(struct vmm_vmx_interrupt_vcpu *, uint32_t,
    uint32_t, int);
static void vmm_vmx_apicv_deliver(struct vmm_vmx_interrupt_vcpu *, uint8_t);
...
static uint32_t vmm_vmx_apicv_read(const struct vmm_vmx_interrupt_vcpu *,
    uint32_t);
static void vmm_vmx_apicv_write(const struct vmm_vmx_interrupt_vcpu *,
    uint32_t, uint32_t);
...
static void vmm_vmx_ioapic_reassert(struct vmm_vmx_interrupt_machine *,
    uint8_t);
static void vmm_vmx_apicv_vcpu_reset(struct vmm_vmx_interrupt_vcpu *);
static void vmm_vmx_apicv_vcpu_reset_lapic(
    struct vmm_vmx_interrupt_vcpu *);

const struct vmm_vmx_interrupt_ops vmm_vmx_apicv_interrupt_ops = {
	.name = "hardware",
	.machine_create = vmm_vmx_apicv_machine_create,
	.machine_enable = vmm_vmx_apicv_machine_enable,
	.irq_raise_msi = vmm_vmx_apicv_irq_raise_msi,
	.irq_set = vmm_vmx_apicv_irq_set,
	...
	.vcpu_mmio = vmm_vmx_apicv_vcpu_mmio,
	.vcpu_io = vmm_vmx_apicv_vcpu_io,
	.vcpu_msr = vmm_vmx_apicv_vcpu_msr,
	.machine_destroy = vmm_vmx_apicv_machine_destroy,
	.vcpu_create = vmm_vmx_apicv_vcpu_create,
	.vcpu_get_lapic = vmm_vmx_interrupt_vcpu_get_lapic,
	.vcpu_set_lapic = vmm_vmx_interrupt_vcpu_set_lapic,
	.vcpu_destroy = vmm_vmx_apicv_vcpu_destroy,
	.vcpu_prepare = vmm_vmx_apicv_vcpu_prepare,
	.vcpu_enter = vmm_vmx_apicv_vcpu_enter,
	.vcpu_leave = vmm_vmx_apicv_vcpu_leave,
	.vcpu_event_result = vmm_vmx_apicv_vcpu_event_result,
	.vintr_internal = true,
	.vcpu_exit = vmm_vmx_apicv_vcpu_exit,
};

static int
vmm_vmx_apicv_machine_create(struct vmm_machine *machine,
    struct vmm_vmx_interrupt_machine **result)
{
	struct vmm_vmx_interrupt_machine *apicv;
	int error;
	uint32_t pin;

	*result = NULL;
	apicv = os_mem_zalloc(sizeof(*apicv));
	if (apicv == NULL)
		return ENOMEM;
	lwkt_token_init(&apicv->token, "vmmirq");
	apicv->vmspace = machine->vmspace;
	apicv->ioapic_id = 1;
	for (pin = 0; pin < VMM_VMX_IOAPIC_PINS; ++pin)
		apicv->ioapic_redir[pin] = VMM_VMX_IOAPIC_REDIR_MASKED;

	/* VMX requires a private host page for the APIC-access memslot. */
	apicv->apic_access_page = vm_page_alloc(NULL, (vm_pindex_t)ticks,
	    VM_ALLOC_NORMAL | VM_ALLOC_SYSTEM | VM_ALLOC_FORCE_ZERO);
	if (apicv->apic_access_page == NULL) {
		error = ENOMEM;
		goto fail;
	}
	apicv->apic_access_page->valid = VM_PAGE_BITS_ALL;
	vm_page_wire(apicv->apic_access_page);
	vm_page_wakeup(apicv->apic_access_page);
	apicv->apic_access_page_pa = VM_PAGE_TO_PHYS(apicv->apic_access_page);

	*result = apicv;
	return 0;

fail:
	vmm_vmx_apicv_machine_destroy(apicv);
	return error;
}

static int
vmm_vmx_apicv_machine_enable(struct vmm_vmx_interrupt_machine *machine)
{
	if (machine == NULL)
		return EINVAL;
	if (machine->apic_access_mapped)
		return EALREADY;

	/* Reserve the architectural LAPIC hole as VMX's APIC-access memslot. */
	pmap_enter(os_vmspace_pmap(machine->vmspace), VMM_VMX_APICV_LAPIC_BASE,
	    machine->apic_access_page, VM_PROT_READ | VM_PROT_WRITE, 0, NULL);
	machine->apic_access_mapped = true;
	return 0;
}

static int
vmm_vmx_apicv_irq_raise_msi(struct vmm_vmx_interrupt_machine *machine,
    uint64_t address, uint32_t data)
{
	...
	if (!logical && destination > VMM_VMX_APICV_MAX_APIC_ID)
		return ENOENT;
	delivered = false;
	...
	if (!logical) {
		...
		if (target != NULL) {
			if (nmi) {
				...
				vmm_vmx_apicv_deliver(target, (uint8_t)vector);
			}
			delivered = true;
		}
	} else {
		for (id = 0; id <= VMM_VMX_APICV_MAX_APIC_ID; ++id) {
			...
			if (target == NULL || (destination &
			    (vmm_vmx_apicv_read(target, VMM_VMX_APIC_LDR) >> 24)) == 0)
				continue;
			if (nmi) {
				atomic_set_int(&target->nmi_pending, 1);
				vmm_vcpu_interrupt(target->vcpu);
...
static int
vmm_vmx_apicv_irq_set(struct vmm_vmx_interrupt_machine *machine,
...
static int
vmm_vmx_apicv_vcpu_io(struct vmm_vcpu *vcpu,
...
static void
vmm_vmx_apicv_machine_destroy(struct vmm_vmx_interrupt_machine *machine)
{
	if (machine == NULL)
		return;
	if (machine->apic_access_mapped) {
		pmap_remove(os_vmspace_pmap(machine->vmspace),
		    VMM_VMX_APICV_LAPIC_BASE,
		    VMM_VMX_APICV_LAPIC_BASE + PAGE_SIZE);
	}
	if (machine->apic_access_page != NULL) {
		vm_page_busy_wait(machine->apic_access_page, FALSE, "vmmvap");
		vm_page_unwire(machine->apic_access_page, 0);
		vm_page_free(machine->apic_access_page);
	}
	os_mem_free(machine, sizeof(*machine));
}

static int
vmm_vmx_apicv_vcpu_create(struct vmm_vmx_interrupt_machine *machine,
    ...
    struct vmm_vmx_interrupt_config *config)
{
	...
	*result = NULL;
	if (!vcpu->machine->irqchip)
		return 0;
	if (vcpu->id > VMM_VMX_APICV_MAX_APIC_ID)
		return E2BIG;
	...
	soft->apic_base = VMM_VMX_APICV_LAPIC_BASE |
	...
	if (soft->apic_id != 0)
		vmm_vmx_apicv_vcpu_reset(soft);
	else
		vmm_vmx_apicv_vcpu_reset_lapic(soft);
	lwkt_gettoken(&machine->token);
	...
	lwkt_reltoken(&machine->token);
	config->enabled = true;
	config->virtual_apic_page = soft->apic_page_pa;
	config->apic_access_page = machine->apic_access_page_pa;
	for (uint32_t index = 0; index < nitems(config->eoi_exit); ++index)
		config->eoi_exit[index] = UINT64_MAX;
	*result = soft;
	return 0;
}

static void
vmm_vmx_apicv_vcpu_reset_lapic(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVT_THERMAL,
	    VMM_VMX_APIC_LVT_MASKED);
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVT_PERF,
	    VMM_VMX_APIC_LVT_MASKED);
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVT0,
	    vcpu->apic_id == 0 ? VMM_VMX_APIC_LVT_EXTINT :
	    VMM_VMX_APIC_LVT_MASKED);
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVT1,
	    VMM_VMX_APIC_LVT_MASKED);
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVT_ERROR,
	    VMM_VMX_APIC_LVT_MASKED);
}

static void
vmm_vmx_apicv_vcpu_reset(struct vmm_vmx_interrupt_vcpu *vcpu)
{
...
static void
vmm_vmx_apicv_vcpu_destroy(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	if (vcpu == NULL)
		return;
	lwkt_gettoken(&vcpu->timer_token);
...
static void
vmm_vmx_apicv_vcpu_enter(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	for (word = 7; word >= 1; --word) {
		...
		if ((vector & 0xf0U) <= vmm_vmx_lapic_ppr(vcpu))
			return;
		/* Virtual-interrupt delivery consumes the virtual APIC IRR itself. */
		vmm_vmx_vcpu_set_rvi(vcpu->vcpu, vector);
		return;
	}
	vmm_vmx_vcpu_set_rvi(vcpu->vcpu, 0);
}

static void
vmm_vmx_apicv_vcpu_leave(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	(void)vcpu;
}

static void
vmm_vmx_apicv_vcpu_event_result(struct vmm_vmx_interrupt_vcpu *vcpu,
    bool reinjected)
{
	if (vcpu == NULL || !vcpu->delivery_pending || reinjected)
		return;
	/* This tracks only a PIC virtual-wire injection. */
	vcpu->delivery_pending = false;
}

static int
vmm_vmx_apicv_vcpu_exit(struct vmm_vmx_interrupt_vcpu *vcpu,
    uint64_t exitcode, uint64_t exitinfo1, uint64_t exitinfo2)
{
	...
	if (vcpu == NULL)
		return 0;
	if (exitcode == VMM_VMX_EXIT_APIC_WRITE) {
		/* VMX has already written the virtual-APIC page. */
		register_offset = exitinfo1 & 0xff0U;
		value = vmm_vmx_apicv_read(vcpu, register_offset);
		if (register_offset == VMM_VMX_APIC_ICR_LOW)
			return vmm_vmx_apicv_route_icr(vcpu, value,
			    vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_ICR_HIGH), 0);
		if (register_offset == VMM_VMX_APIC_ICR_HIGH)
			return 1;
		return vmm_vmx_lapic_write(vcpu, register_offset, value) == 0;
	}
	if (exitcode == VMM_VMX_EXIT_VEOI) {
		/* Hardware removed this vector from ISR before the trap-like exit. */
		vmm_vmx_ioapic_reassert(vcpu->machine, exitinfo1 & 0xffU);
		return 1;
	}
	(void)exitinfo2;
	return 0;
...
static uint32_t
vmm_vmx_apicv_read(const struct vmm_vmx_interrupt_vcpu *vcpu, uint32_t reg)
{
    ...
    const void *registers, size_t size)
{
	...
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_VERSION,
	    VMM_VMX_APIC_VERSION_VALUE);
	vcpu->timer_lvtt = vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_LVTT) &
	    VMM_VMX_APIC_LVT_TIMER_VALID;
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_LVTT, vcpu->timer_lvtt);
	vcpu->timer_tdcr = vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_TDCR) &
	    VMM_VMX_APIC_TIMER_DIVIDE_VALID;
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_TDCR, vcpu->timer_tdcr);
	divide = ((vcpu->timer_tdcr & 0x3U) |
	    ((vcpu->timer_tdcr & 0x8U) >> 1)) + 1;
	vcpu->timer_divisor = 1U << (divide & 0x7U);
	value = vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_TMICT);
...
static void
vmm_vmx_apicv_write(const struct vmm_vmx_interrupt_vcpu *vcpu, uint32_t reg,
    uint32_t value)
{
    ...
    uint32_t count)
{
	...
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_TMICT, count);
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_TMCCT, count);
	if (count == 0 ||
    ...
    uint64_t now)
{
	...
	if (!vcpu->timer_active || now < vcpu->timer_deadline_tsc)
		return false;
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_TMCCT, 0);
	...
	vmm_vmx_apicv_write(vcpu, VMM_VMX_APIC_TMCCT,
	    vcpu->timer_tmict);
	return true;
}
...
vmm_vmx_lapic_timer_check(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	lwkt_reltoken(&vcpu->timer_token);
	if (deliver)
		vmm_vmx_apicv_deliver(vcpu, vector);
}
    ...
    uint32_t *value)
{
	...
	if (reg != VMM_VMX_APIC_TMCCT) {
		*value = vmm_vmx_apicv_read(vcpu, reg);
		return 0;
	}
	now = rdtsc();
	lwkt_gettoken(&vcpu->timer_token);
...
vmm_vmx_lapic_ppr(struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	tpr = vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_TPR);
	for (word = 7; word >= 0; --word) {
		isr = (volatile uint32_t *)((uint8_t *)vcpu->apic_page +
		    VMM_VMX_APIC_ISR_BASE + word * 0x10);
...
static int
vmm_vmx_apicv_vcpu_msr(struct vmm_vmx_interrupt_vcpu *vcpu, bool write,
    uint32_t msr, uint64_t *value)
{
	...
	if ((*value & ~VMM_VMX_APICV_APICBASE_VALID) != 0 ||
	    (*value & VMM_VMX_APICBASE_ADDRESS) != VMM_VMX_APICV_LAPIC_BASE)
		return EINVAL;
    ...
    uint32_t value)
{
	...
	case VMM_VMX_APIC_LDR:
		value &= 0xff000000U;
		vmm_vmx_apicv_write(vcpu, reg, value);
		return 0;
...
vmm_vmx_lapic_enabled(const struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	return (vcpu->apic_base & VMM_VMX_APICBASE_ENABLED) != 0 &&
	    (vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_SVR) &
	    VMM_VMX_APIC_SVR_ENABLE) != 0;
}

/*
 * The legacy PIC reaches a hardware-enabled local APIC through LINT0 ExtINT.
 * SVR hardware enable controls LAPIC IRR delivery and does not gate this wire.
 */
static bool
vmm_vmx_lapic_accepts_pic(const struct vmm_vmx_interrupt_vcpu *vcpu)
{
	...
	if ((vcpu->apic_base & VMM_VMX_APICBASE_ENABLED) == 0)
		return true;
	lvt0 = vmm_vmx_apicv_read(vcpu, VMM_VMX_APIC_LVT0);
...
static void
vmm_vmx_apicv_deliver(struct vmm_vmx_interrupt_vcpu *vcpu, uint8_t vector)
{
	volatile uint32_t *irr;
...
static int
vmm_vmx_apicv_vcpu_mmio(struct vmm_vmx_interrupt_vcpu *vcpu,
    uint64_t address,
    bool write, uint32_t *value)
{
	...
	/* LAPIC accesses use the VMCS virtual-APIC page, not an EPT handler. */
	if (address >= VMM_VMX_APICV_LAPIC_BASE &&
	    address < VMM_VMX_APICV_LAPIC_BASE + PAGE_SIZE)
		return ENOENT;
...
static int
vmm_vmx_apicv_route_icr(struct vmm_vmx_interrupt_vcpu *source,
    uint32_t low, uint32_t high, int x2apic)
{
	...
	lwkt_gettoken(&machine->token);
	source_dfr = vmm_vmx_apicv_read(source, VMM_VMX_APIC_DFR);
	for (id = 0; id <= VMM_VMX_APICV_MAX_APIC_ID; ++id) {
		target = machine->targets[id];
		if (target == NULL)
			continue;
		...
		if (shorthand == 0) {
			if ((low & VMM_VMX_APIC_ICR_DEST_LOGICAL) == 0) {
				...
				target_ldr = vmm_vmx_apicv_read(target,
				    VMM_VMX_APIC_LDR) >> 24;
