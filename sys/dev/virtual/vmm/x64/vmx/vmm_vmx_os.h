 ...
 *
 * DragonFly support used by the imported Intel VMX backend.
 *
...
#define os_mtx_init(lock) lockinit(lock, "vmm_vmx", 0, 0)
...
#define os_preempt_disabled() (td_nest_count != 0)
...
#define os_atomic_load_uint(value) atomic_load_int(value)
#define os_atomic_inc_64(value) atomic_add_64(value, 1)
#define os_cpuset_init(set) \
	({ *(set) = kmalloc(sizeof(**(set)), M_VMM, M_WAITOK | M_ZERO); })
#define os_cpuset_destroy(set) kfree((set), M_VMM)
#define os_cpuset_isset(set, cpu) CPUMASK_TESTBIT(*(set), (cpu))
#define os_cpuset_clear(set, cpu) ATOMIC_CPUMASK_NANDBIT(*(set), (cpu))
#define os_cpuset_setrunning(set) ATOMIC_CPUMASK_ORMASK(*(set), smp_active_mask)
...
#define x86_set_cr0(value) load_cr0(value)
#define x86_set_cr2(value) load_cr2(value)
#define x86_set_cr4(value) load_cr4(value)
...
static inline void
os_ipi_unicast(os_cpu_t *cpu, void (*function)(void *), void *arg)
{
	cpumask_t mask;

	CPUMASK_ASSBIT(mask, cpu->gd_cpuid);
	lwkt_cpusync_simple(mask, function, arg);
}

/* DragonFly kernel threads remain pinned while preemption is disabled. */
#define curlwp_bind() ((int)0)
#define curlwp_bindx(bound) do { (void)(bound); } while (0)
