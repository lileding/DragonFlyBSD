...
static uint32_t
nvgsp_vmm_child_handle(const struct nvgsp_client *client, uint32_t base)
{
	return (base | (client->object.handle & 0x00000fffu));
}

static uint32_t
nvgsp_vmm_pd2_idx(uint64_t va)
{
...
static int
nvgsp_vmm_pt_alloc(struct nvgsp_state *gsp, struct nvgsp_vmm_pt *pt)
{
	return (nvgsp_bar1_alloc_page_kind(gsp, &pt->page,
	    NVGSP_VRAM_VMM_PT, pt));
}

static void
nvgsp_vmm_pt_free(struct nvgsp_state *gsp, struct nvgsp_vmm_pt *pt)
{
	nvgsp_bar1_free_page(gsp, &pt->page);
}
...
{
	nvgsp_bar1_set_region64(gsp, page->bar1_gva, 0,
...
nvgsp_vmm_pd1_base_page(struct nvgsp_vmm *vmm, uint32_t pd2_idx)
{
	...
	pd1->pd2_idx = pd2_idx;
	error = nvgsp_bar1_alloc_page_kind(gsp, &pd1->page,
	...
	nvgsp_bar1_wr64(gsp, vmm->pt[1].page.bar1_gva + pd2_idx * 8,
...
nvgsp_vmm_pd0_find(struct nvgsp_vmm *vmm, uint32_t pd2_idx, uint32_t pd1_idx)
{
...
static int
nvgsp_vmm_pd0_get(struct nvgsp_vmm *vmm, uint32_t pd2_idx, uint32_t pd1_idx,
    struct nvgsp_vmm_pd0 **out)
{
	...
	pd0->pd2_idx = pd2_idx;
	pd0->pd1_idx = pd1_idx;
	error = nvgsp_bar1_alloc_page_kind(gsp, &pd0->page,
	...
	nvgsp_bar1_wr64(gsp, pd1_page->bar1_gva + pd1_idx * 8,
...
static void
nvgsp_vmm_pd0_write_child_slot(struct nvgsp_state *gsp,
    const struct nvgsp_vmm_pd0 *pd0, uint32_t pd0_idx, uint64_t big_pde,
    uint64_t small_pde)
{
	nvgsp_bar1_wr64(gsp, pd0->page.bar1_gva + (pd0_idx * 2 + 0) * 8,
	    big_pde);
	nvgsp_bar1_wr64(gsp, pd0->page.bar1_gva + (pd0_idx * 2 + 1) * 8,
	    small_pde);
}
    ...
    struct nvgsp_vmm_user_pt **out)
{
	...
	pd2_idx = nvgsp_vmm_pd2_idx(va);
	pd1_idx = nvgsp_vmm_pd1_idx(va);
	pd0_idx = nvgsp_vmm_pd0_idx(va);
	...
	error = nvgsp_vmm_pd0_get(vmm, pd2_idx, pd1_idx, &pd0);
	if (error != 0)
		return (error);
	...
	error = nvgsp_bar1_alloc_page_kind(gsp, &pt->lpt,
	    NVGSP_VRAM_VMM_PT, pt);
	if (error != 0)
		goto fail;
	error = nvgsp_bar1_alloc_page_kind(gsp, &pt->spt,
	    NVGSP_VRAM_VMM_PT, pt);
	if (error != 0)
		goto fail;
	...
	nvgsp_vmm_pd0_write_child_slot(gsp, pd0, pd0_idx,
...
fail:
	if (pt->spt.vram_paddr != 0)
		nvgsp_bar1_free_page(gsp, &pt->spt);
	if (pt->lpt.vram_paddr != 0)
		nvgsp_bar1_free_page(gsp, &pt->lpt);
	kfree(pt, M_NVGSP_VMM);
	return (error);
}

static struct nvgsp_vmm_user_pt *
nvgsp_vmm_user_pt_lookup_va(struct nvgsp_vmm *vmm, uint64_t va)
{
	return (nvgsp_vmm_user_pt_find(vmm, nvgsp_vmm_pd2_idx(va),
	    nvgsp_vmm_pd1_idx(va), nvgsp_vmm_pd0_idx(va)));
}
...
nvgsp_vmm_write_4k_pte(struct nvgsp_vmm *vmm, uint64_t va, uint64_t pte)
{
	...
	error = nvgsp_vmm_user_pt_get(vmm, va, &pt);
	if (error != 0)
		return (error);
	nvgsp_bar1_wr64(vmm->gsp, pt->spt.bar1_gva + nvgsp_vmm_spt_idx(va) * 8,
	    pte);
	return (0);
}
...
nvgsp_vmm_flush(struct nvgsp_vmm *vmm)
{
	if (vmm == NULL || vmm->gsp == NULL)
		return;
	nvgsp_bar1_flush(vmm->gsp);
	nvgsp_vmm_invalidate(vmm);
}

static int
nvgsp_vmm_vaspace_ctor(struct nvgsp_vmm *vmm)
{
	struct nvgsp_vaspace_params *args;
	int error;

	memset(&vmm->vaspace, 0, sizeof(vmm->vaspace));
	args = nvgsp_rm_alloc_get(&vmm->device.object,
	    nvgsp_vmm_child_handle(&vmm->client, NVGSP_RM_VASPACE),
	    FERMI_VASPACE_A, sizeof(*args), &vmm->vaspace);
	if (args == NULL)
		return (ENOMEM);
	args->index = 0;
	args->flags = 0;
	error = nvgsp_rm_alloc_wr(&vmm->vaspace, args);
	if (error != 0)
		memset(&vmm->vaspace, 0, sizeof(vmm->vaspace));
	return (error);
}
...
nvgsp_vmm_copy_pdes(struct nvgsp_vmm *vmm)
{
	...
	ctrl = nvgsp_rm_ctrl_get(&vmm->vaspace,
	...
	reply = ctrl;
	error = nvgsp_rm_ctrl_rd(&vmm->vaspace, &reply, 0);
	return (error);
}

static int
nvgsp_vmm_usermode_ctor(struct nvgsp_vmm *vmm)
{
	struct nvgsp_state *gsp = vmm->gsp;
	void *args;
	int error;
	...
	args = nvgsp_rm_alloc_get(&vmm->device.subdevice,
	    nvgsp_vmm_child_handle(&vmm->client, NVGSP_RM_USERMODE),
...
static int
nvgsp_vmm_ctor(struct nvgsp_state *gsp, uint32_t client_handle,
    struct nvgsp_vmm *vmm)
{
	...
	if (error != 0)
		return (error);
	error = nvgsp_client_ctor(gsp, client_handle, &vmm->client);
	if (error != 0)
		goto fail_sparse_page;
	error = nvgsp_device_ctor(&vmm->client, &vmm->device);
	if (error != 0)
		goto fail_client;

	for (i = 0; i < 3; i++) {
		error = nvgsp_vmm_pt_alloc(gsp, &vmm->pt[i]);
		if (error != 0)
			goto fail_pt;
	}

	nvgsp_bar1_wr64(gsp, vmm->pt[0].page.bar1_gva,
	    nvgsp_pde_to_vram(vmm->pt[1].page.vram_paddr));
	nvgsp_bar1_wr64(gsp, vmm->pt[1].page.bar1_gva,
	    nvgsp_pde_to_vram(vmm->pt[2].page.vram_paddr));
	nvgsp_bar1_flush(gsp);

	error = nvgsp_vmm_vaspace_ctor(vmm);
	if (error != 0)
		goto fail_pt;
	error = nvgsp_vmm_copy_pdes(vmm);
	if (error != 0)
		goto fail_vaspace;
	error = nvgsp_vmm_usermode_ctor(vmm);
	if (error != 0)
		goto fail_vaspace;
...
fail_pt:
	for (i = 0; i < 3; i++)
		nvgsp_vmm_pt_free(gsp, &vmm->pt[i]);
	nvgsp_device_dtor(&vmm->device);
fail_client:
	nvgsp_client_dtor(&vmm->client);
fail_sparse_page:
	nvgsp_dmamem_free(gsp, &vmm->sparse_page);
	return (error);
}
...
nvgsp_vmm_dtor(struct nvgsp_vmm *vmm)
{
	...
	while ((pt = LIST_FIRST(&vmm->user_pt_pages)) != NULL) {
		LIST_REMOVE(pt, link);
		nvgsp_bar1_free_page(vmm->gsp, &pt->spt);
		nvgsp_bar1_free_page(vmm->gsp, &pt->lpt);
		kfree(pt, M_NVGSP_VMM);
	}
	while ((pd0 = LIST_FIRST(&vmm->user_pd0_pages)) != NULL) {
		LIST_REMOVE(pd0, link);
		nvgsp_bar1_free_page(vmm->gsp, &pd0->page);
		kfree(pd0, M_NVGSP_VMM);
	}
	while ((pd1 = LIST_FIRST(&vmm->user_pd1_pages)) != NULL) {
		LIST_REMOVE(pd1, link);
		nvgsp_bar1_free_page(vmm->gsp, &pd1->page);
		kfree(pd1, M_NVGSP_VMM);
	}
	...
	nvgsp_dmamem_free(vmm->gsp, &vmm->sparse_page);
	nvgsp_device_dtor(&vmm->device);
	nvgsp_client_dtor(&vmm->client);
}
...
nvgsp_vmm_unmap(struct nvgsp_vmm *vmm, uint64_t va, uint64_t size)
{
	...
	lwkt_gettoken(&vmm->tok);
	for (off = 0; off < size; off += NVGSP_GMMU_PT_PAGE_SIZE) {
		pt = nvgsp_vmm_user_pt_lookup_va(vmm, va + off);
		if (pt == NULL)
			continue;
		nvgsp_bar1_wr64(vmm->gsp,
		    pt->spt.bar1_gva + nvgsp_vmm_spt_idx(va + off) * 8, 0);
	}
	lwkt_reltoken(&vmm->tok);
	nvgsp_vmm_flush(vmm);
	return (0);
